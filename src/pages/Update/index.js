...
export default class Update extends Component {
  ...
  handleTest = async action => {
    ...
    const result = specs.indexOf(action);
    let data = [];
    console.log(action);
    ...
    if(result > -1){
      const newSpecs = specs.splice(result, 0, action);
      await this.setState({
        specs: [ newSpecs ]
      });
    }
    ...
    const { specs, name, crm, phone, uf, city, specialties } = this.state;

    const req = api.put('/doctor/${doctor_id}', {
      name,
      crm,
      phone,
      state: uf,
